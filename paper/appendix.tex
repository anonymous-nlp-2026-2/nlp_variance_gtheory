...
\section{Projection Validation Details}
...
The effect of $|\Delta G| = 0.035$ estimation error on item budgets is shown in Figure~\ref{fig:budget_sensitivity}. The percentage increase is remarkably stable (23.0--24.5\%, mean 23.2\%), because the relative budget adjustment $\Delta n / n$ depends on the $G$-formula ratio rather than on absolute variance components. Absolute $\Delta n$ ranges from 13 (ARC) to 67 (GSM8K), scaling with the baseline budget.
...
\paragraph{Models.} See Table~\ref{tab:model_ids} (main text) for the eight models. Full HuggingFace identifiers: \texttt{meta-llama/Llama-3.1-8B-Instruct} \cite{grattafiori2024llama3}, \texttt{google/gemma-2-9b-it} \cite{riviere2024gemma2}, \texttt{mistralai/Mistral-7B-Instruct-v0.3} \cite{jiang2023mistral}, \texttt{Qwen/Qwen3-8B} \cite{yang2025qwen3}, \texttt{01-ai/Yi-1.5-9B-Chat} \cite{young2024yi}, \texttt{allenai/OLMo-2-1124-7B-Instruct} \cite{olmo2025olmo2}, \texttt{internlm/internlm2\_5-7b-chat} \cite{cai2024internlm2}, \texttt{deepseek-ai/deepseek-llm-7b-chat} \cite{bi2024deepseek}.
...
All experiments were conducted with Python~3.10, PyTorch~2.1, and CUDA~12.8 on a single NVIDIA RTX PRO 6000 GPU (96\,GB). Inference used vLLM~0.9.2 with HuggingFace Transformers~4.40. Variance component estimation used StatsModels~0.14 and SciPy~1.12. A full list of pinned dependencies is available in the released code repository.
